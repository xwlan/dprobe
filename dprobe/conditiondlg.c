	...
	)
{
	...
	if (!IsIdentical) {
		...
		//
		// If it's identical, free current condition object
		//
	...
	)
{
	...
	Bits = Object; 
	Bits |= (ULONG)(Relation << 8);
	Bits |= (ULONG)(Action << 16);
	return Bits;
}
...
{
	*Object   = Bits & (ULONG)0x000000ff;
	*Relation = (Bits >> 8) & (ULONG)0x000000ff;
	*Action   = (Bits >> 16) & (ULONG)0x000000ff;
}
