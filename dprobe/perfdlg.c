...
typedef enum _PerfCounterType {
	...
	PerfCounterIoTotal,
	PerfCounterStackBuckets,
	PerfCounterTextBuckets,
	PERF_COUNTER_NUMBER,
} PerfCounterType;
	...
	)
{
	...
	item.iItem = PerfCounterStackBuckets;
	item.iSubItem = PerfColumnCounter;
	item.pszText = L"Stack Trace Buckets";
	ListView_InsertItem(hWndCounters, &item);
	...
	item.iItem = PerfCounterTextBuckets;
	item.iSubItem = PerfColumnCounter;
	item.pszText = L"Stack Symbol Buckets";
	ListView_InsertItem(hWndCounters, &item);
	...
	)
{
	...
	item.iSubItem = PerfColumnValue;
	StringCchPrintf(Buffer, MAX_PATH - 1, L"%u", Perf->StackEntryBuckets); 
	item.pszText = Buffer;
	ListView_SetItem(hWndCounters, &item);
	...
	item.iSubItem = PerfColumnValue;
	StringCchPrintf(Buffer, MAX_PATH - 1, L"%u", Perf->StackTextBuckets); 
	item.pszText = Buffer;
	ListView_SetItem(hWndCounters, &item);
