	...
	)
{
	...
	while (ListEntry != &PerfListHead) {
		...
		//
		// N.B. For hosttask dprobe.exe, update its stack trace counters 
		//

		if (Information->HostTask) {
			MspGetStackCounters(&Information->StackEntryBuckets,
								&Information->StackTextBuckets);
		}
