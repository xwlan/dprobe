	...
	)
{
	...
	if (Command->CommandType == CommandProbe) {
		StringCchPrintf(Buffer, MAX_PATH, L"Applying probes to %s...", Process->Name);
	}
	
	if (Command->CommandType == CommandStop) {
		StringCchPrintf(Buffer, MAX_PATH, L"Stopping probes of %s...", Process->Name);
	}

	if (Command->CommandType == CommandStartDebug) {
		StringCchPrintf(Buffer, MAX_PATH, L"Starting process %s...", Process->Name);
	}
...
//
// Packet is a pointer to MSP_USER_COMMAND
