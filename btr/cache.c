...
BTR_CACHE_OBJECT BtrCacheObject;
...
{
	RtlZeroMemory(&BtrCacheObject, sizeof(BTR_CACHE_OBJECT));
	return S_OK;
}
	...
	)
{
	...
	if (BtrCacheObject.IsConfigured) {
		return S_OK;
	}

	//
	// N.B. These handles are duplicated by control side with same
	// access right to specified objects.
	//

	BtrCacheObject.IndexFileHandle = Packet->IndexFileHandle;
	BtrCacheObject.DataFileHandle = Packet->DataFileHandle;
	BtrCacheObject.SharedDataHandle = Packet->SharedDataHandle;

	//
	// Map shared global data into current address space
	//

	BtrSharedData = (PBTR_SHARED_DATA)MapViewOfFile(BtrCacheObject.SharedDataHandle, 
	...
	Status = BtrInitializeCounter(Packet->PerfHandle);
	if (Status != S_OK) {
		BtrUninitializeCache();
		return S_FALSE;
	}

	BtrCacheObject.IsConfigured = TRUE;
	return S_OK;
}
	...
	)
{
	...
	if (BtrCacheObject.SharedDataHandle) {
		CloseHandle(BtrCacheObject.SharedDataHandle);
	}
	if (BtrCacheObject.IndexFileHandle) {
		CloseHandle(BtrCacheObject.IndexFileHandle);
	}
	if (BtrCacheObject.DataFileHandle) {
		CloseHandle(BtrCacheObject.DataFileHandle);
	}
}
	...
	)
{
	...
	__try {
		...
		DataObject->Object = CreateFileMapping(BtrCacheObject.DataFileHandle, NULL, 
		...
		IndexObject->Object = CreateFileMapping(BtrCacheObject.IndexFileHandle, NULL, 
			                                    PAGE_READWRITE, 0, 0, NULL);	
		if (!IndexObject->Object) {
			Status = GetLastError();
			__leave;
	...
	)
{
	...
	Status = S_OK;
	NewLength = FileLength + BTR_SHARED_DATA_FILE_INCREMENT;
	...
	if (CurrentLength > FileLength) {
		...
		Status = BtrExtendFileLength(BtrCacheObject.DataFileHandle, NewLength);
		if (Status == S_OK) {
...
repeat:
	...
	if (IS_FILE_OFFSET(MappingObject, End)) {
		...
		MappedStart = BtrUlong64RoundDown(Start, BTR_SHARED_DATA_MAP_INCREMENT);
		MappedEnd = BtrUlong64RoundUp(End, BTR_SHARED_DATA_MAP_INCREMENT);
	...
	MappingObject->Object = CreateFileMapping(BtrCacheObject.DataFileHandle, NULL, 
		                                      PAGE_READWRITE, 0, 0, NULL);
	...
	)
{
	...
	if (IS_FILE_OFFSET(MappingObject, End)) {
		...
		MappedOffset = BtrUlong64RoundDown(Start, BTR_INDEX_MAP_INCREMENT);
		MappingObject->MappedVa = MapViewOfFile(MappingObject->Object, FILE_MAP_WRITE, 
			                                    (DWORD)(MappedOffset >> 32), (DWORD)MappedOffset, 
											    BTR_INDEX_MAP_INCREMENT);
		if (!MappingObject->MappedVa) {
			return BTR_E_MAPVIEWOFFILE;
		}
		
		MappingObject->MappedOffset = MappedOffset;
		MappingObject->MappedLength = BTR_INDEX_MAP_INCREMENT;

		return BtrWriteSharedCacheIndex(Thread, RecordOffset, Location, Size);
	}
	...
	if (Location < CurrentLength) {
		...
		MappingObject->Object = CreateFileMapping(BtrCacheObject.IndexFileHandle, NULL, 
												  PAGE_READWRITE, 0, 0, NULL);
		if (!MappingObject->Object) {
			return BTR_E_CREATEFILEMAPPING;
		}
	...
	if (Location < BtrSharedData->IndexFileLength) {
		...
		ExtendedLength = BtrSharedData->IndexFileLength + BTR_INDEX_FILE_INCREMENT;
		if (ExtendedLength > BTR_MAXIMUM_INDEX_FILE_SIZE) {
			...
			Status = BtrExtendFileLength(BtrCacheObject.IndexFileHandle, ExtendedLength);
			if (Status == S_OK) {
